...
def length(sequences):
    ...
    seq_len = tf.reduce_sum(used, reduction_indices=1)  # 行加和
...
# print("data load finished")  # 词向量字典载入完毕


class HAN:

    # def __init__(self, vocab_size, num_classes, embedding_size=400, hidden_size=50):
    def __init__(self, num_classes, embedding_size=400, hidden_size=50):
    ...
    def word2vec(self):
        with tf.name_scope("embedding"):   # heying: 这里要改词嵌入向量
